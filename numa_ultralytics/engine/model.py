...
from numa_ultralytics.cfg import TASK2DATA, get_cfg, get_save_dir
from numa_ultralytics.engine.results import Results
from numa_ultralytics.hub import HUB_WEB_ROOT, HUBTrainingSession
from numa_ultralytics.nn.tasks import attempt_load_one_weight, guess_model_task, yaml_model_load
...
class Model(torch.nn.Module):
    ...
    loaded from local files, numa_ultralytics HUB, or Triton Server.
    ...
    Attributes:
        ...
        session (HUBTrainingSession): The numa_ultralytics HUB session, if applicable.
    ...
    def __init__(
        ...
        model sources, including local files, numa_ultralytics HUB models, and Triton Server models. The method
        ...
        Args:
            model (Union[str, Path]): Path or name of the model to load or create. Can be a local file path, a
                model name from numa_ultralytics HUB, or a Triton Server model.
        ...
        # Check if numa_ultralytics HUB model from https://hub.numa_ultralytics.com
    ...
    def __call__(
        ...
        **kwargs: Any,
        ...
        Returns:
            (List[numa_ultralytics.engine.results.Results]): A list of prediction results, each encapsulated in a
                Results object.
    ...
    def is_hub_model(model: str) -> bool:
        """
        Check if the provided model is an numa_ultralytics HUB model.

        This static method determines whether the given model string represents a valid numa_ultralytics HUB model
        identifier.
        ...
        Returns:
            (bool): True if the model is a valid numa_ultralytics HUB model, False otherwise.

        Examples:
            >>> Model.is_hub_model("https://hub.numa_ultralytics.com/models/MODEL")
            True
    ...
    def save(self, filename: Union[str, Path] = "saved_model.pt") -> None:
        ...
        the date, numa_ultralytics version, license information, and a link to the documentation.
        ...
        updates = {
            ...
            "version": __version__,
            "license": "AGPL-3.0 License (https://numa_ultralytics.com/license)",
    ...
    def embed(
        ...
        **kwargs: Any,
        ...
        Examples:
            >>> model = YOLO("yolo11n.pt")
            >>> image = "https://numa_ultralytics.com/images/bus.jpg"
    ...
    def predict(
        ...
        **kwargs: Any,
        ...
        is_cli = (ARGV[0].endswith("yolo") or ARGV[0].endswith("numa_ultralytics")) and any(
    ...
    def track(
        ...
        **kwargs: Any,
        ...
        Returns:
            (List[numa_ultralytics.engine.results.Results]): A list of tracking results, each a Results object.
        ...
        if not hasattr(self.predictor, "trackers"):
            from numa_ultralytics.trackers import register_tracker
    ...
    def val(
        ...
        **kwargs: Any,
        ...
        Args:
            validator (numa_ultralytics.engine.validator.BaseValidator | None): An instance of a custom validator class for
                validating the model.
            **kwargs: Arbitrary keyword arguments for customizing the validation process.

        Returns:
            (numa_ultralytics.utils.metrics.DetMetrics): Validation metrics obtained from the validation process.
    ...
    def benchmark(
        ...
        **kwargs: Any,
        ...
        It uses the 'benchmark' function from the numa_ultralytics.utils.benchmarks module. The benchmarking is
    ...
    def train(
        ...
        **kwargs: Any,
        ...
        from a checkpoint, integrating with numa_ultralytics HUB, and updating model and configuration after training.

        When using numa_ultralytics HUB, if the session has a loaded model, the method prioritizes HUB training
        ...
        """
        self._check_is_pytorch_model()
        if hasattr(self.session, "model") and self.session.model.id:  # numa_ultralytics HUB session with loaded model
            if any(kwargs):
    ...
    def tune(
        ...
        **kwargs: Any,
        ...
        When Ray Tune is enabled, it leverages the 'run_ray_tune' function from the numa_ultralytics.utils.tuner module.
        ...
        self._check_is_pytorch_model()
        if use_ray:
            from numa_ultralytics.utils.tuner import run_ray_tune
    ...
    def names(self) -> Dict[int, str]:
        ...
        using the 'check_class_names' function from the numa_ultralytics.nn.autobackend module. If the predictor is not
        ...
        """
        from numa_ultralytics.nn.autobackend import check_class_names
    ...
    def add_callback(self, event: str, func) -> None:
        ...
        Args:
            event (str): The name of the event to attach the callback to. Must be a valid event name recognized
                by the numa_ultralytics framework.
    ...
    def clear_callback(self, event: str) -> None:
        ...
        Args:
            event (str): The name of the event for which to clear the callbacks. This should be a valid event name
                recognized by the numa_ultralytics callback system.
        ...
        Notes:
            - This method affects both custom callbacks added by the user and default callbacks
              provided by the numa_ultralytics framework.
    ...
    def task_map(self) -> dict:
        ...
        various tasks and modes within the numa_ultralytics framework.
        ...
        Note:
            ...
            classes supported by the numa_ultralytics framework. The docstring provides a general
