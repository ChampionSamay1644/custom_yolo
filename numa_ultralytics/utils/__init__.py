...
from numa_ultralytics import __version__
...
ASSETS_URL = "https://github.com/numa_ultralytics/assets/releases/download/v0.0.0"  # assets GitHub URL
DEFAULT_CFG_PATH = ROOT / "cfg/default.yaml"
DEFAULT_SOL_CFG_PATH = ROOT / "cfg/solutions/default.yaml"  # numa_ultralytics solutions yaml path
NUM_THREADS = min(8, max(1, os.cpu_count() - 1))  # number of YOLO multiprocessing threads
AUTOINSTALL = str(os.getenv("YOLO_AUTOINSTALL", True)).lower() == "true"  # global auto-install mode
VERBOSE = str(os.getenv("YOLO_VERBOSE", True)).lower() == "true"  # global verbose mode
TQDM_BAR_FORMAT = "{l_bar}{bar:10}{r_bar}" if VERBOSE else None  # tqdm bar format
LOGGING_NAME = "numa_ultralytics"
...
HELP_MSG = """
    Examples for running numa_ultralytics:

    1. Install the numa_ultralytics package:
    ...
    3. Use the command line interface (CLI):
        ...
        numa_ultralytics 'yolo' CLI commands use the following syntax:
            ...
            Where   TASK (optional) is one of [detect, segment, classify, pose, obb]
                    ...
                    ARGS (optional) are any number of custom "arg=value" pairs like "imgsz=320" that override defaults.
                        See all ARGS at https://docs.numa_ultralytics.com/usage/cfg or with "yolo cfg"
    ...
    Docs: https://docs.numa_ultralytics.com
    Community: https://community.numa_ultralytics.com
...
class TQDM(rich.tqdm if TQDM_RICH else tqdm.tqdm):
    ...
    Examples:
        >>> from numa_ultralytics.utils import TQDM
    ...
    def __init__(self, *args, **kwargs):
        ...
        This class extends the original tqdm class to provide customized behavior for numa_ultralytics projects.
...
class IterableSimpleNamespace(SimpleNamespace):
    ...
    def __getattr__(self, attr):
        ...
        raise AttributeError(
            f"""
            '{name}' object has no attribute '{attr}'. This may be caused by a modified or out of date numa_ultralytics
            'default.yaml' file.\nPlease update your code with 'pip install -U numa_ultralytics' and if necessary replace
            {DEFAULT_CFG_PATH} with the latest version from
            https://github.com/numa_ultralytics/numa_ultralytics/blob/main/numa_ultralytics/cfg/default.yaml
            """
        )
...
def set_logging(name="LOGGING_NAME", verbose=True):
    ...
    This function configures logging for the numa_ultralytics library, setting the appropriate logging level and
    ...
    Examples:
        >>> set_logging(name="numa_ultralytics", verbose=True)
        >>> logger = logging.getLogger("numa_ultralytics")
...
class ThreadingLocked:
    ...
    Example:
        ```python
        from numa_ultralytics.utils import ThreadingLocked
...
DEFAULT_SOL_DICT = yaml_load(DEFAULT_SOL_CFG_PATH)  # numa_ultralytics solutions configuration
...
def get_user_config_dir(sub_dir="numa_ultralytics"):
    """
...
USER_CONFIG_DIR = Path(os.getenv("YOLO_CONFIG_DIR") or get_user_config_dir())  # numa_ultralytics settings dir
...
class TryExcept(contextlib.ContextDecorator):
    """
    numa_ultralytics TryExcept class. Use as @TryExcept() decorator or 'with TryExcept():' context manager.
...
class SettingsManager(JSONDict):
    """
    SettingsManager class for managing and persisting numa_ultralytics settings.
    ...
    def __init__(self, file=SETTINGS_FILE, version="0.0.6"):
        ...
        from numa_ultralytics.utils.torch_utils import torch_distributed_zero_first
        ...
        self.help_msg = (
            f"\nView numa_ultralytics Settings with 'yolo settings' or at '{self.file}'"
            "\nUpdate Settings with 'yolo settings key=value', i.e. 'yolo settings runs_dir=path/to/dir'. "
            "For help see https://docs.numa_ultralytics.com/quickstart/#numa_ultralytics-settings."
        )
        ...
        with torch_distributed_zero_first(RANK):
            ...
            if not self.file.exists() or not self:  # Check if file doesn't exist or is empty
                LOGGER.info(f"Creating new numa_ultralytics Settings v{version} file ✅ {self.help_msg}")
                self.reset()
    ...
    def _validate_settings(self):
        ...
        if not (correct_keys and correct_types and correct_version):
            LOGGER.warning(
                "WARNING ⚠️ numa_ultralytics settings reset to default values. This may be due to a possible problem "
                f"with your settings or a recent numa_ultralytics package update. {self.help_msg}"
            )
            self.reset()

        if self.get("datasets_dir") == self.get("runs_dir"):
            LOGGER.warning(
                f"WARNING ⚠️ numa_ultralytics setting 'datasets_dir: {self.get('datasets_dir')}' "
    ...
    def update(self, *args, **kwargs):
        ...
        for k, v in kwargs.items():
            if k not in self.defaults:
                raise KeyError(f"No numa_ultralytics setting '{k}'. {self.help_msg}")
            t = type(self.defaults[k])
            if not isinstance(v, t):
                raise TypeError(
                    f"numa_ultralytics setting '{k}' must be '{t.__name__}' type, not '{type(v).__name__}'. {self.help_msg}"
                )
        super().update(*args, **kwargs)
...
def vscode_msg(ext="numa_ultralytics.numa_ultralytics-snippets") -> str:
    """Display a message to install numa_ultralytics-Snippets for VS Code if not already installed."""
    path = (USER_CONFIG_DIR.parents[2] if WINDOWS else USER_CONFIG_DIR.parents[1]) / ".vscode/extensions"
    obs_file = path / ".obsolete"  # file tracks uninstalled extensions, while source directory remains
    installed = any(path.glob(f"{ext}*")) and ext not in (obs_file.read_text("utf-8") if obs_file.exists() else "")
    url = "https://docs.numa_ultralytics.com/integrations/vscode"
    return "" if installed else f"{colorstr('VS Code:')} view numa_ultralytics VS Code Extension ⚡ at {url}"


# Run below code on utils init ------------------------------------------------------------------------------------

# Check first-install steps
PREFIX = colorstr("numa_ultralytics: ")
...
from numa_ultralytics.utils.patches import imread, imshow, imwrite, torch_load, torch_save
