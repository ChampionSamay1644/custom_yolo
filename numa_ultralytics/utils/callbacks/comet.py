...
from numa_ultralytics.utils import LOGGER, RANK, SETTINGS, TESTS_RUNNING, ops
from numa_ultralytics.utils.metrics import ClassifyMetrics, DetMetrics, OBBMetrics, PoseMetrics, SegmentMetrics
...
try:
    ...
    # Names of plots created by numa_ultralytics that are logged to Comet
...
def _get_comet_model_name():
    """Returns the model name for Comet from the environment variable COMET_MODEL_NAME or defaults to 'numa_ultralytics'."""
    return os.getenv("COMET_MODEL_NAME", "numa_ultralytics")
...
def _resume_or_create_experiment(args: SimpleNamespace) -> None:
    ...
    try:
        ...
        experiment.log_other("Created from", "numa_ultralytics")
...
def on_fit_epoch_end(trainer):
    ...
    if curr_epoch == 1:
        from numa_ultralytics.utils.torch_utils import model_info_for_loggers
