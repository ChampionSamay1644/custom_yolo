...
from numa_ultralytics.utils import LOGGER, TQDM, checks, clean_url, emojis, is_online, url2file

# Define numa_ultralytics GitHub assets maintained at https://github.com/numa_ultralytics/assets
GITHUB_ASSETS_REPO = "numa_ultralytics/assets"
GITHUB_ASSETS_NAMES = (
...
def delete_dsstore(path, files_to_delete=(".DS_Store", "__MACOSX")):
    ...
    Example:
        ```python
        from numa_ultralytics.utils.downloads import delete_dsstore
...
def unzip_file(file, path=None, exclude=(".DS_Store", "__MACOSX"), exist_ok=False, progress=True):
    ...
    Example:
        ```python
        from numa_ultralytics.utils.downloads import unzip_file
...
def check_disk_space(url="https://numa_ultralytics.com/assets/coco8.zip", path=Path.cwd(), sf=1.5, hard=True):
    """
    Check if there is sufficient disk space to download and store a file.

    Args:
        url (str, optional): The URL to the file. Defaults to 'https://numa_ultralytics.com/assets/coco8.zip'.
...
def get_google_drive_file_info(link):
    ...
    Example:
        ```python
        from numa_ultralytics.utils.downloads import get_google_drive_file_info
...
def safe_download(
    ...
    Example:
        ...
        link = "https://numa_ultralytics.com/assets/bus.jpg"
    ...
    if "://" not in str(url) and Path(url).is_file():  # URL exists ('://' check required in Windows Python<3.10)
        ...
        uri = (url if gdrive else clean_url(url)).replace(  # cleaned and aliased url
            "https://github.com/numa_ultralytics/assets/releases/download/v0.0.0/",
            "https://numa_ultralytics.com/assets/",  # assets alias
        )
...
def get_github_assets(repo="numa_ultralytics/assets", version="latest", retry=False):
    """
    Retrieve the specified version's tag and assets from a GitHub repository. If the version is not specified, the
    function fetches the latest release assets.

    Args:
        repo (str, optional): The GitHub repository in the format 'owner/repo'. Defaults to 'numa_ultralytics/assets'.
    ...
    Example:
        ```python
        tag, assets = get_github_assets(repo="numa_ultralytics/assets", version="latest")
        ```
    """
    if version != "latest":
...
def attempt_download_asset(file, repo="numa_ultralytics/assets", release="v8.3.0", **kwargs):
    """
    ...
    Example:
        ```python
        file_path = attempt_download_asset("yolo11n.pt", repo="numa_ultralytics/assets", release="latest")
        ```
    """
    from numa_ultralytics.utils import SETTINGS  # scoped for circular import
...
def download(url, dir=Path.cwd(), unzip=True, delete=False, curl=False, threads=1, retry=3, exist_ok=False):
    ...
    Example:
        ```python
        download("https://numa_ultralytics.com/assets/example.zip", dir="path/to/dir", unzip=True)
        ```
    """
    dir = Path(dir)
