...
def parse_requirements(file_path=ROOT.parent / "requirements.txt", package=""):
    ...
    Args:
        ...
        package (str, optional): Python package to use instead of requirements.txt file, i.e. package='numa_ultralytics'.
    ...
    Example:
        ```python
        from numa_ultralytics.utils.checks import parse_requirements

        parse_requirements(package="numa_ultralytics")
        ```
    """
    if package:
...
def check_version(
    ...
    if not current:  # if current is '' or None
        ...
        return True
    elif not current[0].isdigit():  # current is package name rather than version string, i.e. current='numa_ultralytics'
        try:
...
def check_latest_pypi_version(package_name="numa_ultralytics"):
    """
...
def check_pip_update_available():
    """
    Checks if a new version of the numa_ultralytics package is available on PyPI.
    ...
    if ONLINE and IS_PIP_PACKAGE:
        try:
            ...
            if check_version(__version__, f"<{latest}"):  # check if current version is < latest version
                LOGGER.info(
                    f"New https://pypi.org/project/numa_ultralytics/{latest} available 😃 "
                    f"Update with 'pip install -U numa_ultralytics'"
                )
                return True
        except Exception:
            pass
...
def check_font(font="Arial.ttf"):
    ...
    url = f"https://github.com/numa_ultralytics/assets/releases/download/v0.0.0/{name}"
...
def check_requirements(requirements=ROOT.parent / "requirements.txt", exclude=(), install=True, cmds=""):
    ...
    Example:
        ...
        check_requirements("numa_ultralytics>=8.0.0")

        # Check multiple packages
        check_requirements(["numpy", "numa_ultralytics>=8.0.0"])
        ```
    """
    ...
    if s:
        if install and AUTOINSTALL:  # check environment variable
            ...
            LOGGER.info(f"{prefix} numa_ultralytics requirement{'s' * (n > 1)} {pkgs} not found, attempting AutoUpdate...")
            try:
                t = time.time()
        ...
        if "u.yaml" in file:
            ...
            if file != original_file and verbose:
                LOGGER.info(
                    ...
                    f"trained with https://github.com/numa_ultralytics/numa_ultralytics and feature improved performance vs "
                    f"standard YOLOv5 models trained with https://github.com/numa_ultralytics/yolov5.\n"
                )
    return file
...
def collect_system_info():
    ...
    from numa_ultralytics.utils import ENVIRONMENT  # scope to avoid circular import
    from numa_ultralytics.utils.torch_utils import get_cpu_info, get_gpu_info
    ...
    package_info = {}
    for r in parse_requirements(package="numa_ultralytics"):
        try:
...
def check_amp(model):
    ...
    Example:
        ...
        from numa_ultralytics.utils.checks import check_amp
    ...
    """
    from numa_ultralytics.utils.torch_utils import autocast
    ...
    try:
        ...
        LOGGER.warning(
            ...
            f"Unable to load YOLO11n for AMP checks due to possible numa_ultralytics package modifications. {warning_msg}"
        )
...
def print_args(args: Optional[dict] = None, show_file=True, show_func=False):
    ...
    def strip_auth(v):
        """Clean longer numa_ultralytics HUB URLs by stripping potential authentication information."""
