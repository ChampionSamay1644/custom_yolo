...
if WINDOWS and check_version(torch.__version__, "==2.4.0"):  # reject version 2.4.0 on Windows
    LOGGER.warning(
        ...
        "https://github.com/numa_ultralytics/numa_ultralytics/issues/15049"
    )
...
def get_cpu_info():
    ...
    from numa_ultralytics.utils import PERSISTENT_CACHE  # avoid circular import error
...
def select_device(device="", batch=0, newline=False, verbose=True):
    ...
    s = f"numa_ultralytics {__version__} 🚀 Python-{PYTHON_VERSION} torch-{torch.__version__} "
...
def init_seeds(seed=0, deterministic=False):
    ...
    # torch.backends.cudnn.benchmark = True  # AutoBatch problem https://github.com/numa_ultralytics/yolov5/issues/9287
    if deterministic:
        if TORCH_2_0:
...
def strip_optimizer(f: Union[str, Path] = "best.pt", s: str = "", updates: dict = None) -> dict:
    ...
    Example:
        ```python
        from pathlib import Path
        from numa_ultralytics.utils.torch_utils import strip_optimizer
    ...
    Note:
        Use `numa_ultralytics.nn.torch_safe_load` for missing modules with `x = torch_safe_load(f)[0]`
    """
    try:
        ...
        LOGGER.warning(f"WARNING ⚠️ Skipping {f}, not a valid numa_ultralytics model: {e}")
        return {}
...
def profile(input, ops, n=10, device=None, max_num_obj=0):
    """
    numa_ultralytics speed, memory and FLOPs profiler.

    Example:
        ```python
        from numa_ultralytics.utils.torch_utils import profile
