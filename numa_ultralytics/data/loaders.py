...
from numa_ultralytics.data.utils import FORMATS_HELP_MSG, IMG_FORMATS, VID_FORMATS
from numa_ultralytics.utils import IS_COLAB, IS_KAGGLE, LOGGER, ops
from numa_ultralytics.utils.checks import check_requirements
from numa_ultralytics.utils.patches import imread
...
class LoadScreenshots:
    """
    numa_ultralytics screenshot dataloader for capturing and processing screen images.
...
class LoadImagesAndVideos:
    ...
    def __init__(self, path, batch=1, vid_stride=1):
        ...
        for p in sorted(path) if isinstance(path, (list, tuple)) else [path]:
            a = str(Path(p).absolute())  # do not use .resolve() https://github.com/numa_ultralytics/numa_ultralytics/issues/2912
            if "*" in a:
...
def autocast_list(source):
    """Merges a list of sources into a list of numpy arrays or PIL images for numa_ultralytics prediction."""
    files = []
    for im in source:
        if isinstance(im, (str, Path)):  # filename or uri
            ...
            raise TypeError(
                f"type {type(im).__name__} is not a supported numa_ultralytics prediction source type. \n"
                f"See https://docs.numa_ultralytics.com/modes/predict for supported source types."
            )
