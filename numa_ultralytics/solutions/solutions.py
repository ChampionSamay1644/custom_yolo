...
from numa_ultralytics.utils import ASSETS_URL, DEFAULT_CFG_DICT, DEFAULT_SOL_DICT, LOGGER
from numa_ultralytics.utils.checks import check_imshow, check_requirements


class BaseSolution:
    """
    A base class for managing numa_ultralytics Solutions.

    This class provides core functionality for various numa_ultralytics Solutions, including model loading, object tracking,
    ...
    Attributes:
        ...
        model (numa_ultralytics.YOLO): Loaded YOLO model instance.
    ...
    def __init__(self, IS_CLI=False, **kwargs):
        """
        Initializes the `BaseSolution` class with configuration settings and the YOLO model for numa_ultralytics solutions.
        ...
        LOGGER.info(f"numa_ultralytics Solutions: ✅ {DEFAULT_SOL_DICT}")
        ...
        if IS_CLI and self.CFG["source"] is None:
            ...
            safe_download(f"{ASSETS_URL}/{d_s}")  # download source from numa_ultralytics assets
    ...
    def display_output(self, im0):
        ...
        if self.CFG.get("show") and self.env_check:
            cv2.imshow("numa_ultralytics Solutions", im0)
