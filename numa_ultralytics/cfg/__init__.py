...
SOLUTIONS_HELP_MSG = f"""
    Arguments received: {str(["yolo"] + ARGV[1:])}. numa_ultralytics 'yolo solutions' usage overview:

        yolo solutions SOLUTION ARGS

        Where SOLUTION (optional) is one of {list(SOLUTION_MAP.keys())[:-1]}
              ARGS (optional) are any number of custom 'arg=value' pairs like 'show_in=True' that override defaults 
                  at https://docs.numa_ultralytics.com/usage/cfg
    ...
    """
CLI_HELP_MSG = f"""
    Arguments received: {str(["yolo"] + ARGV[1:])}. numa_ultralytics 'yolo' commands use the following syntax:
    ...
    5. numa_ultralytics solutions usage
    ...
    Solutions: https://docs.numa_ultralytics.com/solutions/
...
def check_cfg(cfg, hard=True):
    """
    Checks configuration argument types and values for the numa_ultralytics library.
...
def handle_yolo_hub(args: List[str]) -> None:
    """
    Handles numa_ultralytics HUB command-line interface (CLI) commands for authentication.

    This function processes numa_ultralytics HUB CLI commands such as login and logout. It should be called when executing a
    ...
    Notes:
        - The function imports the 'hub' module from numa_ultralytics to perform login and logout operations.
        - For the 'login' command, if no API key is provided, an empty string is passed to the login function.
        - The 'logout' command does not require any additional arguments.
    """
    from numa_ultralytics import hub

    if args[0] == "login":
        key = args[1] if len(args) > 1 else ""
        # Log in to numa_ultralytics HUB using the provided API key
        hub.login(key)
    elif args[0] == "logout":
        # Log out from numa_ultralytics HUB
        hub.logout()
...
def handle_yolo_settings(args: List[str]) -> None:
    ...
    Notes:
        ...
        - For more information on handling YOLO settings, visit:
          https://docs.numa_ultralytics.com/quickstart/#numa_ultralytics-settings
    """
    url = "https://docs.numa_ultralytics.com/quickstart/#numa_ultralytics-settings"  # help URL
    try:
        if any(args):
            if args[0] == "reset":
        ...
        LOGGER.info(f"💡 Learn more about numa_ultralytics Settings at {url}")
...
def handle_yolo_solutions(args: List[str]) -> None:
    ...
    Args:
        args (List[str]): Command-line arguments for configuring and running the numa_ultralytics YOLO
            solutions: https://docs.numa_ultralytics.com/solutions/, It can include solution name, source,
    ...
    Notes:
        ...
        - The Streamlit app file is located in the numa_ultralytics package directory.
    """
    ...
    if s_n == "inference":
        ...
        LOGGER.info("💡 Loading numa_ultralytics live inference app...")
        subprocess.run(
        ...
        from numa_ultralytics import solutions  # import numa_ultralytics solutions
        ...
        from numa_ultralytics.utils.files import increment_path  # for output directory path update
...
def entrypoint(debug=""):
    """
    numa_ultralytics entrypoint function for parsing and executing command-line arguments.

    This function serves as the main entry point for the numa_ultralytics CLI, parsing command-line arguments and
    ...
    Notes:
        ...
        - For a list of all available commands and their arguments, see the provided help messages and the
          numa_ultralytics documentation at https://docs.numa_ultralytics.com.
    """
    ...
    if mode in {"predict", "track"} and "source" not in overrides:
        overrides["source"] = (
            "https://numa_ultralytics.com/images/boats.jpg" if task == "obb" else DEFAULT_CFG.source or ASSETS
        )
    ...
    # Show help
    LOGGER.info(f"💡 Learn more at https://docs.numa_ultralytics.com/modes/{mode}")
