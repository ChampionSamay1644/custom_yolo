...
from numa_ultralytics import RTDETR, YOLO
from numa_ultralytics.cfg import MODELS, TASK2DATA, TASKS
from numa_ultralytics.data.build import load_inference_source
...
from numa_ultralytics.utils.torch_utils import TORCH_1_9
...
def test_model_profile():
    ...
    from numa_ultralytics.nn.tasks import DetectionModel
...
def test_predict_img(model_name):
    ...
    batch = [
        str(SOURCE),  # filename
        Path(SOURCE),  # Path
        "https://github.com/numa_ultralytics/assets/releases/download/v0.0.0/zidane.jpg" if ONLINE else SOURCE,  # URI
...
def test_track_stream():
    ...
    """
    video_url = "https://github.com/numa_ultralytics/assets/releases/download/v0.0.0/decelera_portrait_min.mov"
    model = YOLO(MODEL)
...
@pytest.mark.skipif(WINDOWS, reason="Windows slow CI export bug https://github.com/numa_ultralytics/numa_ultralytics/pull/16003")
def test_workflow():
...
@pytest.mark.skipif(not ONLINE, reason="environment is offline")
def test_data_utils():
    """Test utility functions in numa_ultralytics/data/utils.py, including dataset stats and auto-splitting."""
    from numa_ultralytics.data.utils import HUBDatasetStats, autosplit
    from numa_ultralytics.utils.downloads import zip_directory

    # from numa_ultralytics.utils.files import WorkingDirectory
    # with WorkingDirectory(ROOT.parent / 'tests'):

    for task in TASKS:
        file = Path(TASK2DATA[task]).with_suffix(".zip")  # i.e. coco8.zip
        download(f"https://github.com/numa_ultralytics/hub/raw/main/example_datasets/{file}", unzip=False, dir=TMP)
...
def test_data_converter():
    ...
    from numa_ultralytics.data.converter import coco80_to_coco91_class, convert_coco

    file = "instances_val2017.json"
    download(f"https://github.com/numa_ultralytics/assets/releases/download/v0.0.0/{file}", dir=TMP)
...
def test_events():
    ...
    from numa_ultralytics.hub.utils import Events
...
def test_cfg_init():
    """Test configuration initialization utilities from the 'numa_ultralytics.cfg' module."""
    from numa_ultralytics.cfg import check_dict_alignment, copy_default_cfg, smart_value
...
def test_utils_init():
    """Test initialization utilities in the numa_ultralytics library."""
    from numa_ultralytics.utils import get_git_branch, get_git_origin_url, get_ubuntu_version, is_github_action_running
...
def test_utils_checks():
    ...
    checks.check_version("numa_ultralytics", "8.0.0")
    checks.print_args()


@pytest.mark.skipif(WINDOWS, reason="Windows profiling is extremely slow (cause unknown)")
def test_utils_benchmarks():
    """Benchmark model performance using 'ProfileModels' from 'numa_ultralytics.utils.benchmarks'."""
    from numa_ultralytics.utils.benchmarks import ProfileModels
...
def test_utils_torchutils():
    ...
    from numa_ultralytics.nn.modules.conv import Conv
    from numa_ultralytics.utils.torch_utils import get_flops_with_torch_profiler, profile, time_sync
...
def test_utils_files():
    ...
    from numa_ultralytics.utils.files import file_age, file_date, get_latest_run, spaces_in_path
...
def test_utils_patches_torch_save():
    ...
    from numa_ultralytics.utils.patches import torch_save

    mock = MagicMock(side_effect=RuntimeError)

    with patch("numa_ultralytics.utils.patches._torch_save", new=mock):
...
def test_nn_modules_conv():
    ...
    from numa_ultralytics.nn.modules.conv import CBAM, Conv2, ConvTranspose, DWConvTranspose2d, Focus
...
def test_nn_modules_block():
    ...
    from numa_ultralytics.nn.modules.block import C1, C3TR, BottleneckCSP, C3Ghost, C3x
...
@pytest.mark.skipif(not ONLINE, reason="environment is offline")
def test_hub():
    """Test numa_ultralytics HUB functionalities (e.g. export formats, logout)."""
    from numa_ultralytics.hub import export_fmts_hub, logout
    from numa_ultralytics.hub.utils import smart_request
...
def test_classify_transforms_train(image, auto_augment, erasing, force_color_jitter):
    ...
    from numa_ultralytics.data.augment import classify_augmentations
