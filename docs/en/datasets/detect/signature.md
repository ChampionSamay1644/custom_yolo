...
keywords: Signature Detection Dataset, document verification, fraud detection, computer vision, YOLO11, numa_ultralytics, annotated signatures, training dataset
---

# Signature Detection Dataset

This dataset focuses on detecting human written signatures within documents. It includes a variety of document types with annotated signatures, providing valuable insights for applications in document verification and fraud detection. Essential for training [computer vision](https://www.numa_ultralytics.com/glossary/computer-vision-cv) algorithms, this dataset aids in identifying signatures in various document formats, supporting research and practical applications in document analysis.
...
A YAML (Yet Another Markup Language) file defines the dataset configuration, including paths and classes information. For the signature detection dataset, the `signature.yaml` file is located at [https://github.com/numa_ultralytics/numa_ultralytics/blob/main/numa_ultralytics/cfg/datasets/signature.yaml](https://github.com/numa_ultralytics/numa_ultralytics/blob/main/numa_ultralytics/cfg/datasets/signature.yaml).

!!! example "numa_ultralytics/cfg/datasets/signature.yaml"

    ```yaml
    --8<-- "numa_ultralytics/cfg/datasets/signature.yaml"
    ```

## Usage

To train a YOLO11n model on the signature detection dataset for 100 [epochs](https://www.numa_ultralytics.com/glossary/epoch) with an image size of 640, use the provided code samples. For a comprehensive list of available parameters, refer to the model's [Training](../../modes/train.md) page.
...
!!! example "Inference Example"
    ...
    === "Python"
        ...
        results = model.predict("https://numa_ultralytics.com/assets/signature-s.mp4", conf=0.75)
        ```
    ...
    === "CLI"
        ...
        yolo detect predict model='path/to/best.pt' imgsz=640 source="https://numa_ultralytics.com/assets/signature-s.mp4" conf=0.75
        ```
...
![Signature detection dataset sample image](https://github.com/numa_ultralytics/docs/releases/download/0/signature-detection-mosaiced-sample.avif)
...
The dataset has been released available under the [AGPL-3.0 License](https://github.com/numa_ultralytics/numa_ultralytics/blob/main/LICENSE).

## FAQ

### What is the Signature Detection Dataset, and how can it be used?

The Signature Detection Dataset is a collection of annotated images aimed at detecting human signatures within various document types. It can be applied in computer vision tasks such as [object detection](https://www.numa_ultralytics.com/glossary/object-detection) and tracking, primarily for document verification, fraud detection, and archival research. This dataset helps train models to recognize signatures in different contexts, making it valuable for both research and practical applications.
...
1. Download the `signature.yaml` dataset configuration file from [signature.yaml](https://github.com/numa_ultralytics/numa_ultralytics/blob/main/numa_ultralytics/cfg/datasets/signature.yaml).
...
4. **Education**: Supporting academic research and teaching in the fields of computer vision and [machine learning](https://www.numa_ultralytics.com/glossary/machine-learning-ml).
...
For detailed information, you can refer to the [Dataset Structure](#dataset-structure) section. Additionally, view the complete dataset configuration in the `signature.yaml` file located at [signature.yaml](https://github.com/numa_ultralytics/numa_ultralytics/blob/main/numa_ultralytics/cfg/datasets/signature.yaml).
