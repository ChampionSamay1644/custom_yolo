---
comments: true
description: Discover a variety of models supported by numa_ultralytics, including YOLOv3 to YOLOv10, NAS, SAM, and RT-DETR for detection, segmentation, and more.
keywords: numa_ultralytics, supported models, YOLOv3, YOLOv4, YOLOv5, YOLOv6, YOLOv7, YOLOv8, YOLOv9, YOLOv10, SAM, NAS, RT-DETR, object detection, image segmentation, classification, pose estimation, multi-object tracking
---

# Models Supported by numa_ultralytics

Welcome to numa_ultralytics' model documentation! We offer support for a wide range of models, each tailored to specific tasks like [object detection](../tasks/detect.md), [instance segmentation](../tasks/segment.md), [image classification](../tasks/classify.md), [pose estimation](../tasks/pose.md), and [multi-object tracking](../modes/track.md). If you're interested in contributing your model architecture to numa_ultralytics, check out our [Contributing Guide](../help/contributing.md).

![numa_ultralytics YOLO11 Comparison Plots](https://raw.githubusercontent.com/numa_ultralytics/assets/refs/heads/main/yolo/performance-comparison.png)
...
3. **[YOLOv5](yolov5.md)**: An improved version of the YOLO architecture by numa_ultralytics, offering better performance and speed trade-offs compared to previous versions.
...
6. **[YOLOv8](yolov8.md)**: The latest version of the YOLO family, featuring enhanced capabilities such as [instance segmentation](https://www.numa_ultralytics.com/glossary/instance-segmentation), pose/keypoints estimation, and classification.
7. **[YOLOv9](yolov9.md)**: An experimental model trained on the numa_ultralytics [YOLOv5](yolov5.md) codebase implementing Programmable Gradient Information (PGI).
...
9. **[YOLO11](yolo11.md) 🚀 NEW**: numa_ultralytics' latest YOLO models delivering state-of-the-art (SOTA) performance across multiple tasks.
...
14. **[YOLO-NAS](yolo-nas.md)**: YOLO [Neural Architecture Search](https://www.numa_ultralytics.com/glossary/neural-architecture-search-nas) (NAS) Models.
15. **[Realtime Detection Transformers (RT-DETR)](rtdetr.md)**: Baidu's PaddlePaddle Realtime Detection [Transformer](https://www.numa_ultralytics.com/glossary/transformer) (RT-DETR) models.
...
<p align="center">
  ...
  <br>
  <strong>Watch:</strong> Run numa_ultralytics YOLO models in just a few lines of code.
</p>
...
Note the below example is for YOLOv8 [Detect](../tasks/detect.md) models for [object detection](https://www.numa_ultralytics.com/glossary/object-detection). For additional supported tasks see the [Segment](../tasks/segment.md), [Classify](../tasks/classify.md) and [Pose](../tasks/pose.md) docs.

!!! example

    === "Python"

        [PyTorch](https://www.numa_ultralytics.com/glossary/pytorch) pretrained `*.pt` models as well as configuration `*.yaml` files can be passed to the `YOLO()`, `SAM()`, `NAS()` and `RTDETR()` classes to create a model instance in Python:
...
Interested in contributing your model to numa_ultralytics? Great! We're always open to expanding our model portfolio.

1. **Fork the Repository**: Start by forking the [numa_ultralytics GitHub repository](https://github.com/numa_ultralytics/numa_ultralytics).
...
### What are the key advantages of using numa_ultralytics YOLOv8 for object detection?

numa_ultralytics YOLOv8 offers enhanced capabilities such as real-time object detection, instance segmentation, pose estimation, and classification. Its optimized architecture ensures high-speed performance without sacrificing [accuracy](https://www.numa_ultralytics.com/glossary/accuracy), making it ideal for a variety of applications. YOLOv8 also includes built-in compatibility with popular datasets and models, as detailed on the [YOLOv8 documentation page](../models/yolov8.md).

### How can I train a YOLOv8 model on custom data?

Training a YOLOv8 model on custom data can be easily accomplished using numa_ultralytics' libraries. Here's a quick example:
...
### Which YOLO versions are supported by numa_ultralytics?

numa_ultralytics supports a comprehensive range of YOLO (You Only Look Once) versions from YOLOv3 to YOLOv10, along with models like NAS, SAM, and RT-DETR. Each version is optimized for various tasks such as detection, segmentation, and classification. For detailed information on each model, refer to the [Models Supported by numa_ultralytics](../models/index.md) documentation.

### Why should I use numa_ultralytics HUB for [machine learning](https://www.numa_ultralytics.com/glossary/machine-learning-ml) projects?

numa_ultralytics HUB provides a no-code, end-to-end platform for training, deploying, and managing YOLO models. It simplifies complex workflows, enabling users to focus on model performance and application. The HUB also offers cloud training capabilities, comprehensive dataset management, and user-friendly interfaces. Learn more about it on the [numa_ultralytics HUB](../hub/index.md) documentation page.
