...
keywords: YOLO-NAS, Deci AI, object detection, deep learning, Neural Architecture Search, numa_ultralytics, Python API, YOLO model, SuperGradients, pretrained models, quantization, AutoNAC
---
...
Experience the power of next-generation object detection with the pre-trained YOLO-NAS models provided by numa_ultralytics. These models are designed to deliver top-notch performance in terms of both speed and accuracy. Choose from a variety of options tailored to your specific needs:
...
Each model variant is designed to offer a balance between [Mean Average Precision](https://www.numa_ultralytics.com/glossary/mean-average-precision-map) (mAP) and latency, helping you optimize your object detection tasks for both performance and speed.

## Usage Examples

numa_ultralytics has made YOLO-NAS models easy to integrate into your Python applications via our `numa_ultralytics` python package. The package provides a user-friendly Python API to streamline the process.

The following examples show how to use YOLO-NAS models with the `numa_ultralytics` package for inference and validation:
...
!!! example
    ...
    This example provides simple inference and validation code for YOLO-NAS. For handling inference results see [Predict](../modes/predict.md) mode. For using YOLO-NAS with additional modes see [Val](../modes/val.md) and [Export](../modes/export.md). YOLO-NAS on the `numa_ultralytics` package does not support training.

    === "Python"

        [PyTorch](https://www.numa_ultralytics.com/glossary/pytorch) pretrained `*.pt` models files can be passed to the `NAS()` class to create a model instance in python:

        ```python
        from numa_ultralytics import NAS
...
- **YOLO-NAS-m**: Offers a balanced approach, suitable for general-purpose [object detection](https://www.numa_ultralytics.com/glossary/object-detection) with higher accuracy.
...
| ---------- | -------------------------------------------------------------------------------------------------- | -------------------------------------- | --------- | ---------- | -------- | ------ |
| YOLO-NAS-s | [yolo_nas_s.pt](https://github.com/numa_ultralytics/assets/releases/download/v8.2.0/yolo_nas_s.pt) | [Object Detection](../tasks/detect.md) | ✅        | ✅         | ❌       | ✅     |
| YOLO-NAS-m | [yolo_nas_m.pt](https://github.com/numa_ultralytics/assets/releases/download/v8.2.0/yolo_nas_m.pt) | [Object Detection](../tasks/detect.md) | ✅        | ✅         | ❌       | ✅     |
| YOLO-NAS-l | [yolo_nas_l.pt](https://github.com/numa_ultralytics/assets/releases/download/v8.2.0/yolo_nas_l.pt) | [Object Detection](../tasks/detect.md) | ✅        | ✅         | ❌       | ✅     |
...
We express our gratitude to Deci AI's [SuperGradients](https://github.com/Deci-AI/super-gradients/) team for their efforts in creating and maintaining this valuable resource for the [computer vision](https://www.numa_ultralytics.com/glossary/computer-vision-cv) community. We believe YOLO-NAS, with its innovative architecture and superior object detection capabilities, will become a critical tool for developers and researchers alike.
...
You can easily integrate YOLO-NAS models into your Python application using the `numa_ultralytics` package. Here's a simple example of how to load a pre-trained YOLO-NAS model and perform inference:
...
- **Quantization-Friendly Basic Block:** Enhanced architecture that improves model performance with minimal [precision](https://www.numa_ultralytics.com/glossary/precision) drop post quantization.
...
Yes, numa_ultralytics provides pre-trained YOLO-NAS models that you can access directly. These models are pre-trained on datasets like COCO, ensuring high performance in terms of both speed and accuracy. You can download these models using the links provided in the [Pre-trained Models](#pre-trained-models) section. Here are some examples:

- [YOLO-NAS-s](https://github.com/numa_ultralytics/assets/releases/download/v8.2.0/yolo_nas_s.pt)
- [YOLO-NAS-m](https://github.com/numa_ultralytics/assets/releases/download/v8.2.0/yolo_nas_m.pt)
- [YOLO-NAS-l](https://github.com/numa_ultralytics/assets/releases/download/v8.2.0/yolo_nas_l.pt)
